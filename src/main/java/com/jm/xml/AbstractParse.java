    ...
    public  AbstractParse(String data)
	 ...
	 public Hashtable getTable()
	 {
	 	...
	 	for(Object key:currentMap.keySet())
	 	{
	 		...
	 		newTable.put(key,Util.removeQ(val));
	 	}		
	    return  newTable;	 
	 }
	 ...
	 {   
	     ...
	     _isSpace=Util.isSpace(currentChar);
	 ...
	 {	 while(true)
	        { 
	          char c=getChar0();  
	          if (c==NC.CEND) {return;}
	          if (!Util.isSpace(c)) {currentPos--;return;}
	 ...
	 { 
	   ...
	   currentMap.put(Util.trim(key), Util.trim(value));
	 ...
	 {
		 String sb="";
		 boolean  isSpace=Util.trim(pat).length()==0;
